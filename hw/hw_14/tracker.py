...
class Tracker:
    ...
    def prev_detections(self):
        ...
        """
        detections_dict = {}
        for history_detections in self.detection_history[-self.lookup_tail_size:]:
            for detection in history_detections:
                detections_dict[detection[0]] = detection
    ...
    def bind_tracklet(self, detections):
        ...
        for detection in detections:
            ...
            for prev_detection in prev_detections:
                ious += [iou_score(prev_detection[1:], detection[1:])]

            bind_id = -1
            if len(ious) > 0:
                bind_id = prev_detections[np.argmax(ious)][0]

            if bind_id not in busy_ids:
                busy_ids.add(bind_id)
                detection[0] = bind_id
            else:
                detection[0] = self.new_label()
